...
1. **Check that you can run shell commands.** Run `git --version` and show the student the output. If you cannot execute commands, because you are a chat assistant without a terminal or because tool use is off, stop here. Tell the student that this setup needs a CLI coding agent with shell access, such as Claude Code, Codex CLI or Gemini CLI, opened in a terminal, and that pasting the same prompt there is all they have to do. Do not describe, simulate or narrate the steps below, and do not ask the student to run commands on your behalf.
